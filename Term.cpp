...
void Term::transToJS () {
    std::string value;
    if (subtype == Number) {
        std::cout << kind << " " << subtype << " " << number << std::endl;
    } else if(subtype == VarName || kind == Name) {
        std::cout << kind << " " << subtype << " " << name << std::endl;
    } else if(kind == Command || kind == Expr || kind == BoolExpr) {
        std::cout << kind << " " << subtype << " " << "No" << std::endl;
    } else {
        std::cout << kind << " " << "No" << " " << "No" << std::endl;
    }
