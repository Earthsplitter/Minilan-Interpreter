...
void Term::transToJS (std::string &t) {
    if (subtype == Number) {
        t += std::to_string(kind) + " " + std::to_string(subtype) + " " + std::to_string(number) + "\n";
    } else if(subtype == VarName || kind == Name) {
        t += std::to_string(kind) + " " + std::to_string(subtype) + " " + name + "\n";
    } else if(kind == Command || kind == Expr || kind == BoolExpr) {
        t += std::to_string(kind) + " " + std::to_string(subtype) + " " + "No\n";
    } else {
        t += std::to_string(kind) + " " + "No" + " " + "No\n";
    }

    if (sons.begin() != sons.end()) {
        t += "ChildStart ";
        for(std::list<Term*>::iterator i=sons.begin();i!=sons.end();i++) {
            (*i)->transToJS(t);
        }
...
void Method(const FunctionCallbackInfo<Value>& args) {
    ...
    String::Utf8Value utfValue(file);
    std::string file_source = std::string(*utfValue);

    std::ifstream input(file_source);
    Term *t=parse(input);
    std::string out = "";
    out += "TransStart\n";
    t->transToJS(out);
